...
const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${process.env.BACKEND_URL}/api/v1/course/instructor/courses/${courseId}`, {
          headers: {
            Authorization: `${token}`,
          },
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await axios.put(
        `${process.env.BACKEND_URL}/api/v1/course/instructor/courses/${courseId}`,
        formDataToSend,
        {
          withCredentials: true,
          headers: {
  ...
  const handleLectureUpdate = async (updatedLectures) => {
    try {
      const response = await axios.put(
        `${process.env.BACKEND_URL}/api/v1/course/instructor/${courseId}/lectures`,
